package models.mondo

import models.monster.{Monster, OriginZone}
import scala.util.Random

class Mondo(val currentZone: OriginZone):
    ...
    if monster.originZone == currentZone then
      currentZone match
        case OriginZone.Forest =>
          ...
          monster.attributes = monster.attributes.copy(
        ...
        case OriginZone.Swamp =>
          ...
          monster.attributes = monster.attributes.copy(
            weaknesPhysical = monster.attributes.weaknesPhysical * physicalWeaknessBuff,
            weaknesMagic = monster.attributes.weaknesMagic * magicWeaknessBuff
          )
