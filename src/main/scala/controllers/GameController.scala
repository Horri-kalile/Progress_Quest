...
/** Main game controller that manages the overall game flow and timing.
  *
  * GameController coordinates between the game models (player, monsters, events) and the user interface, handling
  * automatic event triggering, combat sequences, and game state management. It runs the main game loop that drives all
  * gameplay.
  */
object GameController:
  ...
  /** Starts a new game with the given player. Initializes the game loop and updates the UI.
    *
    * @param player
  ...
  private def startGameLoop(): Unit =
    ...
    gameTimer.foreach(_.scheduleAtFixedRate(
        new TimerTask {
  ...
  /** Displays combat steps one by one with 500ms delays for dramatic effect. Updates UI after each step and handles
    * game over conditions.
    *
    * @param steps
    *   List of combat steps (player state, monster state, message)
    * @param finalPlayer
    *   The final player state after all steps
    */
  ...
  /** Gets the current player state.
    *
    * @return
    *   Option containing the current player, or None if no game is active
    */
  ...
  /** Checks if the game is currently running.
    *
    * @return
    *   true if game loop is active, false otherwise
    */
