...
object RandomFunctions:
  ...
  /** Extension method to cap a value between a minimum and maximum bound.
    *
    * @return
    *   the capped value between min and max
    */
  extension (x: Double)
    def capped(min: Double, max: Double): Double = x.max(min).min(max)

  /** Returns a random [[EventType]] based on base probabilities, adjusted dynamically by the player's `lucky`
    * attribute.
    *
    ...
    *
    * @param lucky
    *   The player's `lucky` attribute (0–100+)
    * @return
    *   A randomly chosen [[EventType]] with luck-adjusted weighting
    */
