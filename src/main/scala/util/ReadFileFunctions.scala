...
/** Loads item name data from a JSON file, saved in a resource folder,into a list of strings.
  */
object ItemNameLoader:
  ...
  /** Loads item names from the given file path.
    *
    * @param path
    *   File path to items JSON (default: assets/items.json)
    * @return
    *   List of item names as strings
    */
  def loadItemNames(path: String = "/assets/items.json"): List[String] =
    Using.resource(Source.fromInputStream(getClass.getResourceAsStream(path))) { source =>
      val raw = source.mkString
      val parsed = read[ItemNames](raw)
      parsed.items
    }

/** Loads mission data from a JSON file into a list of MissionData entries. Expected JSON format: { "missions": [ {...},
  * {...} ] }
  */
object MissionLoader:
  ...
  /** Loads all missions from the specified JSON file.
    *
    * @param path
    *   File path to missions JSON (default: assets/missions.json)
    * @return
    *   List of MissionData objects
    */
...
/** Loads equipment names grouped by EquipmentSlot from JSON. Special handling for "Jewelry" key: maps to both Jewelry1
  * and Jewelry2. Expected format: { Head[...], "Jewelry": [...] }
  */
object EquipmentNameLoader:
  /** Loads equipment names and assigns them to matching equipment slots.
    *
    * @param path
    *   File path to equipment JSON (default: assets/equipments.json)
    * @return
    *   Map from EquipmentSlot to list of item names
    */
...
/** Loads skill name data from a JSON file into a SkillNameData structure. Expected format: { "physicalSkills": [...],
  * "magicSkills": [...], ... }
  */
object SkillLoader:
  ...
  /** Loads skill names from JSON and parses into a SkillNameData object.
    *
    * @param path
    *   File path to skills JSON (default: assets/skills.json)
    * @return
    *   SkillNameData with categorized skill names
    */
...
/** Loads monster names grouped by category from JSON. Expected format: { "Forest": [...], "Volcano": [...], ... }
  */
object MonsterLoader:
  ...
  /** Loads monster name data from a JSON file.
    *
    * @param path
    *   File path to monsters JSON (default: assets/monsters.json)
    * @return
    *   Map from zone/type to list of monster names
    */
