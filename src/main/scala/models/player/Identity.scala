...
object PlayerBonusesApplication:
  ...
  /** Applies race- and class-specific bonuses to a player.
    ...
    *   The original Player
    * @return
    *   A new Player with bonuses applied
    */
  def applyRaceAndClassBonuses(player: Player): Player =
    ...
    val (hpMulti, mpMulti, updatedPlayer) = player.identity.race match
      ...
      case Elf =>
        (0.6, 1.4,
          player.withBaseAttributes(
            player.baseAttributes.copy(dexterity = player.baseAttributes.dexterity + Random.between(1, 5))
          ))
      case Dwarf =>
        (1.5, 0.5,
          player.withBaseAttributes(
            player.baseAttributes.copy(constitution = player.baseAttributes.constitution + Random.between(1, 5))
          ))
      case Orc =>
        (1.7, 0.3,
          player.withBaseAttributes(player.baseAttributes.copy(wisdom = player.baseAttributes.wisdom + Random.between(1,
              5))))
      case Gnome =>
        (0.8, 1.2,
          player.withBaseAttributes(
            player.baseAttributes.copy(intelligence = player.baseAttributes.intelligence + Random.between(1, 5))
          ))
      case Titan =>
        (1.6, 0.4,
          player.withBaseAttributes(
            player.baseAttributes.copy(strength = player.baseAttributes.strength + Random.between(1, 5))
          ))
      case PandaMan =>
        (0.8, 1.3,
          player.withBaseAttributes(player.baseAttributes.copy(lucky = player.baseAttributes.lucky + Random.between(1,
              5))))
      case Gundam =>
        (1.3, 0.7,
          player.withBaseAttributes(player.baseAttributes.copy(wisdom = player.baseAttributes.wisdom + Random.between(1,
                3), strength = player.baseAttributes.strength + Random.between(1, 3))))
