...
object EquipmentModule:
  ...
  /** Representation of a piece of equipment.
    ...
    * @param statBonus
    *   Attribute bonuses granted
    * @param value
    *   Computed value (usually sum of attributes)
    */
  ...
  trait EquipmentFactory:
    ...
    /** Generates a random piece of equipment if drop conditions are met.
      *
      * @param probabilityDrop
      *   Base drop chance (0.0 - 1.0)
      * @param playerLucky
      *   Player's luck stat affecting drop chance
      * @param playerLevel
      *   Player's level for attribute scaling
      * @return
      *   Some(Equipment) if drop occurs, None otherwise
      */
  ...
  object EquipmentFactory:
    ...
    /** CCreates a factory that drops equipment based on probability and luck.
      *
      * @param playerLucky
      *   player's luck stat boosting drop chance
      * @param playerLevel
      *   player's level for attribute scaling
      * @return
      *   optionally dropped equipment
      */
    ...
    /** Enhances an equipment item by randomly increasing some of its attributes.
      ...
      * @return
      *   New enhanced equipment instance
      */
  ...
  private case class AlwaysDropFactory() extends EquipmentFactory with EquipmentFactoryHelpers:
    ...
    override def generateRandomEquipment(probabilityDrop: Double, playerLucky: Int, playerLevel: Int)
        : Option[Equipment] =
      for
        slot <- randomSlot()
