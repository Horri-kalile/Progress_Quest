...
/** Enumeration of monster types. Each type represents a different category of creatures with distinct characteristics,
...
/** Data structure for organizing monster names by zones. Used by MonsterLoader to map zone names to lists of
  * appropriate monster names.
  *
  * @param zones
  *   Map from zone name strings to lists of monster names available in that zone
  */
...
/** Represents a monster entity in the game with complete stats, rewards, and behavior.
  *
  * Monsters are the primary opponents that players encounter during their adventures. Each monster has unique
  * attributes, belongs to a specific zone, and provides rewards when defeated. Monsters can have special states like
  * berserk or regenerating that affect their combat behavior.
  *
  * @param name
  *   The display name of the monster
  * @param level
  *   The monster's level, affecting stats and rewards
  * @param monsterType
  *   The category/type of monster (Beast, Undead, etc.)
  * @param originZone
  *   The world zone where this monster originates
  * @param attributes
  *   Combat statistics including HP, attack, defense, and weaknesses
  * @param goldReward
  *   Amount of gold awarded when monster is defeated
  * @param experienceReward
  *   Amount of experience points awarded when defeated
  * @param itemReward
  *   Optional consumable item dropped when defeated
  * @param equipReward
  *   Optional equipment piece dropped when defeated
  * @param behavior
  *   AI behavior pattern that controls monster combat decisions
  * @param description
  *   Flavor text describing the monster's appearance or nature
  * @param berserk
  *   Whether the monster is in a berserk state (increased aggression)
  * @param regenerating
  *   Whether the monster can regenerate health over time
  */
case class Monster(
    name: String,
    level: Int,
  ...
  /** Applies damage to the monster, reducing its current HP. HP cannot go below 0.
    *
    * @param amount
    *   The amount of damage to apply
    * @return
    *   A new Monster instance with reduced HP
    */
  ...
  /** Heals the monster, increasing its current HP. HP cannot exceed the monster's maximum HP.
    *
    * @param amount
    *   The amount of healing to apply
    * @return
    *   A new Monster instance with increased HP
    */
  ...
  /** Calculates the damage dealt when the monster explodes or uses special attacks. Currently, returns the monster's
    * base attack value.
    *
    * @return
    *   The amount of explosion damage the monster can deal
    */
...
/** Factory object for creating monsters with appropriate stats and rewards.
  *
  * This object handles the procedural generation of monsters based on player level, zone requirements, and random
  * factors. It manages monster name loading, attribute scaling, reward calculation, and zone-specific buffs.
  */
object MonstersFactory:
  /** Preloaded monster names organized by zone for consistent creature spawning. Loaded once at startup from external
    * data files.
    */
  ...
  /** Generates a random monster appropriate for the specified zone and player stats.
    *
    * Creates a monster with level and attributes scaled to provide appropriate challenge for the player. Optionally
    * creates "strong" monsters with enhanced stats and better rewards for special encounters.
    *
    * @param zone
    *   The world zone where the monster will appear
    * @param playerLevel
    *   Player's current level for stat scaling
    * @param playerLucky
    *   Player's luck attribute affecting reward generation
    * @param strong
    *   Whether to create a powerful version with enhanced stats
    * @return
    *   A fully configured Monster instance ready for combat
    */
  def randomMonsterForZone(zone: OriginZone, playerLevel: Int, playerLucky: Int, strong: Boolean = false): Monster =
    ...
    val rewards: (Int, Int, Option[Item], Option[Equipment]) =
      generateRewards(monsterLevel, playerLevel, playerLucky, strong, monsterType)
  ...
  /** Calculates appropriate monster level based on player level and encounter type.
    *
    * Regular monsters are within 1 level of the player (±1). Strong monsters are 1-2 levels above the player for
    * increased challenge. Minimum level is always 1.
    *
    * @param playerLevel
    *   The player's current level
    * @param strong
    *   Whether this is a strong encounter
    * @return
    *   Scaled monster level appropriate for the encounter
    */
  ...
  /** Generates monster combat attributes scaled to the monster's level.
    *
    * Strong monsters have reduced weaknesses (0.5-1.0 multiplier, taking less damage) while regular monsters have
    * standard or increased weaknesses (1.0-1.5 multiplier). All stats scale linearly with monster level.
    *
    * @param level
    *   The monster's level
    * @param strong
    *   Whether this is a strong monster with enhanced stats
    * @return
    *   MonsterAttributes with HP, attack, defense, and damage weaknesses
    */
  ...
  /** Applies monster type-specific modifications to base attributes.
    ...
    *
    * @param attrs
    *   Base MonsterAttributes to modify.
    * @param t
    *   MonsterType specifying which modifiers to apply.
    * @return
    *   Modified MonsterAttributes with type-specific adjustments.
    */
  private def applyTypeModifiers(attrs: MonsterAttributes, t: MonsterType): MonsterAttributes = t match
    case MonsterType.Undead => attrs.copy(weaknessMagic = (attrs.weaknessMagic * 1.5).min(2.0),
        weaknessPhysical = (attrs.weaknessPhysical * 0.8).max(0.5))
  ...
  /** Calculates rewards granted by a monster based on its level, type, player stats, and strength.
    *
    * Rewards include gold, experience points, and optional item drops. MonsterType influences multipliers on gold and
    * XP, and increases item drop chances.
    *
    * @param level
    *   Monster's level for base reward scaling.
    * @param playerLevel
    *   Player's level to scale equipment drops.
    * @param playerLucky
    *   Player's luck attribute, increasing drop chances.
    * @param strong
    *   Whether monster is a strong variant doubling rewards.
    * @param monsterType
    *   The monster's type affecting reward profile.
    * @return
    *   Tuple of (gold: Int, experience: Int, item: Option[Item], equipment: Option[Equipment])
    */
  private def generateRewards(level: Int, playerLevel: Int, playerLucky: Int, strong: Boolean, monsterType: MonsterType)
    ...
    val randomItem: Option[Item] =
      ItemFactory.probBasedCreate(baseDropChance + itemDropBonus).createRandomItem(playerLucky)
