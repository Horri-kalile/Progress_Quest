...
/**
 * MonsterBehavior represents different special abilities that monsters
 * can exhibit during combat. Each behavior applies specific modifications to a monster's
 * base attributes, creating varied and strategic combat encounters. Behaviors are applied
 * when monsters are created and can affect stats, special states, or combat mechanics.
 *
 * The apply method transforms a base monster into a specialized variant with enhanced
 * or modified capabilities based on the specific behavior pattern.
 */
sealed trait MonsterBehavior:
  /**
   * Applies this behavior's modifications to a monster.
   *
   * @param monster The base monster to modify
   * @return A new Monster instance with behavior-specific modifications applied
   */
...
case object Aggressive extends MonsterBehavior:
   ...
   *
   * @param monster The monster to make aggressive
   * @return Monster with boosted attack attribute
   */
  def apply(monster: Monster): Monster =
...
case object Defensive extends MonsterBehavior:
   ...
   *
   * @param monster The monster to make defensive
   * @return Monster with boosted defense attribute
   */
  def apply(monster: Monster): Monster =
    monster.copy(attributes = monster.attributes.copy(
...
case object MoreHp extends MonsterBehavior:
   ...
   *
   * @param monster The monster to give more health
   * @return Monster with increased HP values
   */
  def apply(monster: Monster): Monster =
 ...
 *
 * Berserk monsters have the berserk flag set to true, which may trigger special
 * combat behaviors or interactions during battle.
 */
case object Berserk extends MonsterBehavior:
  /**
   * Sets the monster's berserk state to true.
   *
   * @param monster The monster to make berserk
   * @return Monster with berserk flag enabled
   */
  def apply(monster: Monster): Monster =
 ...
 *
 * Regenerating monsters have the regenerating flag set to true, allowing them
 * to recover health over time during extended battles.
 */
case object Regenerating extends MonsterBehavior:
  /**
   * Sets the monster's regenerating state to true.
   *
   * @param monster The monster to give regeneration
   * @return Monster with regenerating flag enabled
   */
 ...
 *
 * This object manages the collection of all available monster behaviors and
 * provides methods for randomly selecting behaviors during monster generation,
 */
object MonsterBehavior:
  /**
   * Complete list of all available monster behaviors for random selection.
   * Used by the monster generation system to create varied encounters.
   */
   ...
   *
   * @return A randomly chosen MonsterBehavior for use in monster generation
   */
