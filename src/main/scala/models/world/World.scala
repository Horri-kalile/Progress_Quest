...
/** World management system that handles zone-based mechanics and environmental effects.
  *
  * The World object provides functionality for managing the game world's different zones and their effects on monsters
  * and gameplay. It handles zone transitions, applies environmental buffs to monsters when they are in their native
  * zones, and provides descriptive information about each zone's characteristics.
  *
  * Key features:
...
object World:
  ...
  /** Selects a random zone different from the current zone for world transitions.
    *
    * This method is used when the player moves to a new area, ensuring they don't stay in the same zone.
    *
    * @param currentZone
    *   The zone the player is currently in
    * @return
    *   A randomly selected OriginZone different from the current zone
    */
  ...
  /** Applies environmental buffs to monsters when they are in their native zone.
    *
    * Monsters receive significant advantages when encountered in their origin zone, Each zone provides different types
    * of buffs based on its environmental characteristics. Monsters outside their native zone receive no environmental
    * bonuses.
    *
    * Zone-specific buffs:
    ...
    *
    * @param monster
    *   The monster to potentially buff
    * @param currentZone
    *   The zone where the encounter is taking place
    * @return
    *   A new Monster instance with zone buffs applied if applicable
    */
  ...
  /** Provides descriptive text explaining each zone's environmental effects.
    *
    * Returns user-friendly descriptions that explain what advantages monsters gain when encountered in their native
    * zones. This information is displayed in the UI to help players understand the tactical implications of different
    *
    * @param zone
    *   The zone to get a description for
    * @return
    *   A descriptive string explaining the zone's effects on monsters
    */
